$(document).ready(function(){
    ...
    $('#edit-sub-form').on('submit',function(e){
        ...
        if(updatesubject){
            $.ajax({
                url:"/agency/descriptive_update_subject/",
    ...
    $(document).on('click','.delete-btn',function(){
        ...
        swal({
            ...
            if (isConfirm){
                $.ajax({
                    url:"/agency/descriptive_delete_subject/",
                    headers: {'X-CSRFToken': $("input[name=csrfmiddlewaretoken]").val() },
                    type:'POST',
                    contentType: 'application/json; charset=UTF-8',
    ...
    $('#add-sub-form').on('submit',async function(e){
        ...
        var url = "/agency/descriptive_view/" + createSubId['subject_id'];
            ...
            var tabClone = `<div class="col-12 col-sm-6 col-md-12 col-lg-6 col-xl-3 sub-card">
                <div class="descriptive_questions">
                        ...
                        <div class="descriptive_questions_action">
                            ...
                            <a class="i_color" href="/agency/descriptive_view/`+createSubId['subject_id']+`" target="_blank"><i class="fas fa-eye"></i></a>

                            <a class="i_color" href="/agency/descriptive_add/`+createSubId['subject_id']+`" target="_blank"><i class="fas fa-plus"></i></a>`
...
async function add_sub(add_data){
        var return_data = await $.ajax({
            url:"/agency/descriptive_add_subject/",
            headers: {'X-CSRFToken': $("input[name=csrfmiddlewaretoken]").val() },
            type:'POST',
            contentType: 'application/json; charset=UTF-8',
