$(document).ready(function(){
    ...
    $('#edit-sub-form').on('submit',function(e){
        ...
        if(updatesubject){
            $.ajax({
                url:"/company/image_update_subject/",
    ...
    $(document).on('click','.delete-btn',function(){
        ...
        swal({
            ...
            if (isConfirm){
                $.ajax({
                    url:"/company/image_delete_subject/",
                    headers: {'X-CSRFToken': $("input[name=csrfmiddlewaretoken]").val() },
                    type:'POST',
                    contentType: 'application/json; charset=UTF-8',
    ...
    $('#add-sub-form').on('submit',async function(e){
        ...
        var url = "/company/image_based_question_view/" + createSubId['subject_id'];
        ...
        <div class="descriptive_questions">
            ...
            <div class="descriptive_questions_action">
                <a class="i_color" href="/company/image_based_question_view/`+createSubId['subject_id']+`" target="_blank"><i class="fas fa-eye"></i></a>

                <a class="i_color" href="/company/image_based_question_add/`+createSubId['subject_id']+`" target="_blank"><i class="fas fa-plus"></i></a>` 
                if (delete_=='True'){
