$(document).ready(function(){
    ...
    $('#edit-sub-form').on('submit',function(e){
        ...
        if(updatesubject){
            $.ajax({
                url:"/company/mcq_update_subject/",
    ...
    $(document).on('click','.delete-btn',function(){
        ...
        swal({
            ...
            if (isConfirm){
                $.ajax({
                    url:"/company/mcq_delete_subject/",
                    headers: {'X-CSRFToken': $("input[name=csrfmiddlewaretoken]").val() },
                    type:'POST',
                    contentType: 'application/json; charset=UTF-8',
    ...
    $('#add-sub-form').on('submit',async function(e){
        ...
        var url = "/company/preview_mcq/" + createSubId['subject_id'];
        var tabClone = `<div class="fx-mini-card card">
                            ...
                            <div class="fx-right">`
                            if (edit=='True'){
                               tabClone+=`<div class="edit-btn" data-id="`+createSubId['subject_id']+`" data-name="`+createSubId['subject_name']+`"><i class="fas fa-edit"></i></div>`
                               }
                            if(delete_=='True'){
                               tabClone+=` <div class="delete-btn"><i class="fas fa-trash-alt"></i></div>`
                               }
                           tabClone+=` </div>
                        </div>`;
            ...
            var tabClone =`<div class="fx-mini-card card">
                        ...
                        <div class="fx-col title-field subject_name_e">
                            <input type="text" name="card-id" value="`+createSubId['subject_id']+`" hidden readonly> 
                            <input type="text" name="card-title" value="`+createSubId['subject_name']+`" class="form-control form-control-sm disabledEditText" placeholder="add subject title" readonly>`
                            if (edit=='True'){
                                tabClone+=` <div class="edit-btn" data-id="`+createSubId['subject_id']+`" data-name="`+createSubId['subject_name']+`">
                        ...
                        <div class="fx-col bind-sm-row pt-3 pb-3">
                            ...
                            <div class="fx-right">
                                <a class="i_color" href="/company/preview_mcq/`+createSubId['subject_id']+`" target="_blank"><i class="fas fa-eye"></i></a>

                                <a class="i_color" href="/company/add_mcq/`+createSubId['subject_id']+`" target="_blank"><i class="fas fa-plus"></i></a>`                                                 
