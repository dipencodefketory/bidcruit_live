...
async function getUsers(){
    ...
    var return_data = await $.ajax({
        ...
        error: function (request, status, error) {
            ...
            console.log('parse users >>>>>>>>', JSON.parse(response))
    })
    return return_data
}
var	dbConnection = false;
	...
	$('.slideClose').on('click', function(){ //slide close btn event
		...
		$(".main-content.singlemenu").css('position','initial');
...
/****
 * Quick Side panal slider Functions
 */
 function quickSlideView(slideKey){
	$(".tb-slider-container").each(function(){
		if($(this).data('preview') == slideKey){
			...
			//$('.side-barContainer').css('height',containerHeight+'px');
			$(".main-content.singlemenu").css('position','fixed');
		}else{
			$(this).hide();
		}
	})
