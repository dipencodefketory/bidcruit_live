...
//        $('.edit_subject_name').val($(this).data('name'));
    ...
    $('#edit-sub-form').on('submit',function(e){
        ...
        if(updatesubject){
            $.ajax({
                url:"/agency/coding_update_subject/",
    ...
    $(document).on('click','.delete-btn',function(){
        ...
        swal({
            ...
            if (isConfirm){
                $.ajax({
                    url:"/agency/coding_delete_subject/",
                    headers: {'X-CSRFToken': $("input[name=csrfmiddlewaretoken]").val() },
                    type:'POST',
                    contentType: 'application/json; charset=UTF-8',
    ...
    $('#add-sub-form').on('submit',async function(e){
        ...
        var url = "/agency/coding_category_all/" + createSubId['subject_id'];
